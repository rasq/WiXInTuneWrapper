...
needle
   ...
   .on('readable', function() {
      ...
      $.ajax({
               type: 'GET',
               url: "https://www.udemy.com/api-2.0/users/me/subscribed-wrappers?page_size=50",
               beforeSend: function(){
                   $(".ui.dashboard .wrappers.dimmer").addClass('active');
               },
               headers: header,
               success: function(response) { 
                  $(".ui.dashboard .wrappers.dimmer").removeClass('active');
                  if(response.results.length){
                     $.each(response.results,function(index,course){
                        $('.ui.dashboard .ui.wrappers.items').append(`
                     ...
                     if(response.count>5){
                      $('.ui.wrappers.items').prepend(`
                        <div class="ui fluid search">
                          <div class="ui fluid icon input">
                            <input class="prompt" type="text" placeholder="Filter wrappers">
                            <i class="search icon"></i>
                          </div>
                        </div>
                        `).find('input').keyup(function(){
                          $('.ui.wrappers.items .item').hide();
                          $('.ui.wrappers.items .item .coursename:contains('+this.value+')').parents('.course.item').show();
                        });
                    }
                    if(response.next){
                      $('.ui.wrappers.items').append('<button class="ui basic blue fluid load-more button" data-url='+response.next+'>Load More</button>');
                    }
                  }else{
                     $('.ui.dashboard .wrappers').append(`<div class="ui yellow message">${translate("You have not enrolled in any course")}</div>`)
                  }
               }
            });
      }else{
...
$course.find('.download-status').show();
      $.ajax({
               type: 'GET',
               url: `https://www.udemy.com/api-2.0/wrappers/${courseid}/cached-subscriber-curriculum-items?page_size=100000`,
               beforeSend: function(){
                  ...
                  $.each(response.results, function(i,v){
                    if(v._class=="chapter"){ 
                      ...
                      function  getLecture(lecturename,chapterindex,lectureindex){
                          $.ajax({
                             type: 'GET',
                             url: `https://www.udemy.com/api-2.0/users/me/subscribed-wrappers/${courseid}/lectures/${v.id}?fields[lecture]=view_html,asset,supplementary_assets`,
                             headers: header,
                             success: function(response) {
                                ...
                                if(response.supplementary_assets.length&&!downloadVideosOnly){
                                  ...
                                  $.each(response.supplementary_assets, function(a,b){
                                      $.ajax({
                                        type: 'GET',
                                        url: `https://www.udemy.com/api-2.0/users/me/subscribed-wrappers/${courseid}/lectures/${v.id}/supplementary-assets/${b.id}?fields[asset]=download_urls,external_url,asset_type`,
                                        headers: header,
                                        success: function(response) { 
...
$('.about-sidebar').click(function(){
  $('.content .ui.wrappers,.content .ui.settings').hide();
